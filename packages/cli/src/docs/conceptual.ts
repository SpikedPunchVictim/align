...
- \`align init\` seeds the baseline from the first check. Interactive: asks. CI/non-interactive: requires \`--accept-existing\`, else exits red — silence is never consent. It also refuses, changing nothing, to seed or reset the baseline when the check errored or couldn't resolve all dependencies (pass \`--allow-incomplete\` for the latter) — an incomplete scan proves nothing was fixed.
- \`align baseline accept [--rule <ruleId>]\` accepts current violations (optionally scoped to one rule). \`align baseline show\` lists what's baselined; \`align baseline prune\` drops entries for violations that no longer exist (and refuses, changing nothing, when the check errored — an incomplete scan proves nothing was fixed). Deleting a baseline entry destroys an accepted consent decision, so \`prune\` **asks before deleting**: interactively it prompts, and non-interactively it refuses unless you pass \`--yes\`. A run that deletes nothing is never gated.
- \`prune\` deletes an entry only when the scan actually looked at its file. If the file sat behind something the scan couldn't see — an \`excludes\` pattern, a nested checkout, an unreadable directory — the entry is **retained and the reason printed**, because absence from a scan is not evidence of deletion (ADR 028). To forfeit retained entries for a subtree you've excluded on purpose, name it: \`align baseline prune --forget-unscanned <prefix>\`. There is no bare form — the prefix is required, and it deletes accepted consent decisions under that path only.
