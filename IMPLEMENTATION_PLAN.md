...
**Live `align_propose_rules` session (user-run, 2026-07-12): PASS, with one HIGH finding.** The client agent ran the two-pass flow correctly (classification → proposals → grounding + dry-run → human confirmation honored, nothing auto-applied), noticed the just-promoted `metric.loc` and correctly ruled it out for route-handler thinness (sub-path scoping gap), and refused to force approximations for 3 of 4 prose sections with sound reasoning (sdd = scanner-extension decision, not a rule; workspace membership = doctor advisory + policy call; build artifacts = excludes/naming territory). Outcome: kluster ruleset left unchanged, four sections logged as confirmed rule-kind gaps. **HIGH finding (false-green #3)**: grounding accepted a `custom.host` proposal whose `hostRuleName` matches no predicate defined in align.config.ts — its "adds 0 violations" dry-run was vacuous; check-time evaluator suspected to share the hole. Fix landed (064edaf): root cause was deeper than grounding — `custom.host` has NO execution mechanism in v1 (the kind literally counted as a PASSING rule pre-fix); now unregistered predicates hard-error at check and flag as ungroundable at propose; custom.host is schema-only until a registration surface ships. The three-fix pattern is codified as the **reference-validity invariant** (ADR 008 amendment): every name an IR rule references must resolve at check time or the gate errors — enforced plugin-independently in the orchestrator's pre-evaluation guard, with an exhaustive switch the compiler forces new rule kinds to extend. Polish items logged: propose-diff should separate provenance-only changes from structural ones (agent-attached `because` text made 10 byte-identical rules show "changed"); scanner extension-coverage question (.mjs/.cjs invisible — kluster has real .mjs files); duplicated-error-UI constraint confirmed inexpressible (weak additional evidence for reserved naming/duplication kinds).
...
Tool-wrapping gate stack (format/lint/types/tests, `security.secrets`, `security.tool` — the superseded stage designs below are the reference material, governed by ADR 008's `dependsOn` contract), caching promotion (~10 s trigger), `align watch`, DX backlog, second language plugin → `@align/plugin-api` extraction. Everything promotes on evidence. **`security.manifest` (two rules — source-hygiene, new-dependency) already shipped ahead of this stage, 2026-07-12, ADR 013** — a manifest-only gate needing no tool-wrapping infrastructure; install-script exposure remains here pending a content-pattern classifier rework (ADR 013's follow-up ladder). **Status**: In Progress (2 parallel Sonnet agents, 2026-07-12): batch 1 = `align skill` (schema-generated, --install, MCP instructions field) + `align` bin packaging; batch 2 = external-edge retention (3 demand signals; unlocks the previously-inexpressible child_process predicate as dogfood) + .mjs/.cjs scanner coverage + doctor advisory grouping/caps + propose-diff provenance-only separation.
...
**Promotion log** (see also docs/proposals/rule-expansion-evaluation.md, 23-item evaluation 2026-07-12: 1 promote-now, 11 reserve-with-trigger, 11 reject — zero new rule kinds cleared the bar): **custom.host registration surface — PROMOTED and SHIPPED 2026-07-12** (infrastructure, not a rule kind; evidence = 064edaf's finding that the kind was schema-only). `align.config.ts`'s `hostRules` export + `c.custom.host(name)` register/reference a `HostPredicate`; `GateOrchestrator` executes it via an injected `HostPredicateRegistry` (CLI composition root extracts it from the loaded config); `validateHostRules` now receives the real registered-name set instead of an always-empty one; a predicate that throws surfaces as gate `error` (`HostPredicateExecutionError`), never a silent pass. Dogfooded on align's own repo: `typesLayerIsLeaf` (registered in align.config.ts) enforces that `packages/core/src/types/` — align's foundation layer — never depends on a sibling subdirectory of `packages/core/src/`, a sub-path-scoped invariant `arch.layers`/`arch.no-dependency` can't express at whole-component granularity (§A.2.2's gap); first run found zero violations (the invariant already held), so its value is regression protection, not an existing-debt find. Tests 289→311 (+22, all green), core/plugin-typescript/agent/cli all typecheck clean, `pnpm check` green. **manifest-security probe** approved & in progress (Stage-S-shaped, offline manifest rules vs align/kluster/n8n — evidence-gathering BEFORE any security-rule promotion, per doctrine).

**`security.manifest` gate PROMOTED 2026-07-12, user-approved** — two rules on probe evidence (spike/MANIFEST_PROBE_REPORT.md): (1) **new-dependency-added gate** (real historical catch: `@anthropic-ai/sdk` entering align's own tree in Stage 4, zero false positives) and (2) **dependency-source hygiene** (git/http/file/link specifiers; 3/3 verified-real on n8n). Install-script exposure follows only after content-pattern classifier rework. **Doctrine resolution (user decision)**: install-dependent manifest rules are a CI/CD-context concern — pipelines enforce full install; when `node_modules` is absent such rules report an explicit skipped/advisory state, NEVER a false zero/green (the probe's false-zero lesson). Rejected on evidence: version-pinning policy, registry-URL allowlisting (wrap lockfile-lint if ever), offline dependency-confusion as scoped.

**`security.manifest` gate SHIPPED 2026-07-12** (ADR 013, `docs/adr/013-security-manifest-gate.md`) — `security.manifest.source-hygiene` + `security.manifest.new-dependency`, both first-class `RuleIR`/`Violation` kinds with full DSL (`c.security.manifest.sourceHygiene()` / `.newDependencyGate()`) and tier-2 doc-authoring support, a new `security` gate in `GateOrchestrator` (`dependsOn: []`, always runs, independent of `architecture`/`parse` — ADR 008's always-run carve-out), and a new manifest scan domain in `@align/plugin-typescript` (root + workspace `package.json` + `pnpm-lock.yaml`, reusing `workspace.ts`'s `loadWorkspacePackages`; core stays pure, only owns the `ManifestScanner` injection interface). `new-dependency` is re-expressed through align's existing baseline-consent machinery (ADR 006) rather than a git-history diff — stateless, fingerprinted on (manifest path, dependency name), name-level only. Dogfooded on align's own repo (26 current deps across 5 manifests seeded into baseline via `align baseline accept`, `pnpm check` green); a temp fixture proved the add-a-dep → red (violation naming the dep + declaring manifest) → `baseline accept` → green loop end-to-end. Validated read-only against `test-apps/kluster` (27 manifests, 0 source-hygiene, 201 new-dependency day-one seed candidates) and `test-apps/n8n` (72 manifests, 3/3 source-hygiene finds reproducing the probe exactly — both `xlsx` CDN-tarball sites plus `wa-sqlite`'s git pin — 1,861 new-dependency day-one seed candidates; the `npm:zod-from-json-schema@^0.0.5` alias correctly did not fire). Suite 311→347 passing (+36) + 1 skipped, all four packages typecheck clean, `pnpm check` green.

**Prior promotions**: `arch.metric` (max-LOC) **PROMOTED from reserve 2026-07-12, user-approved** — evidence from the kluster encompassing-ruleset exercise: two confirmed 2,100+-line files (`build-worker.ts` 2,109, a route file 2,220) structurally invisible to all 19 dependency/cycle rules; the encoded standards were incomplete without it. Scope of promotion: the `loc` metric only — `fan-in`/`fan-out`/`instability` remain reserved pending their own evidence. `arch.naming` remains in reserve (the 9-file duplicated-error-UI finding is a duplication smell, not a naming case).
