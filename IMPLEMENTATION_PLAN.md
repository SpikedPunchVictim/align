...
**Promotion log** (see also docs/proposals/rule-expansion-evaluation.md, 23-item evaluation 2026-07-12: 1 promote-now, 11 reserve-with-trigger, 11 reject — zero new rule kinds cleared the bar): **custom.host registration surface** approved & in progress (infrastructure, not a rule kind; evidence = 064edaf's finding that the kind was schema-only); **manifest-security probe** approved & in progress (Stage-S-shaped, offline manifest rules vs align/kluster/n8n — evidence-gathering BEFORE any security-rule promotion, per doctrine).
